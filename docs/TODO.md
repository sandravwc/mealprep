...
- [x] Photo quality feedback: model returns `hinweis` (unscharf, zu dunkel, zu weit weg, Etiketten abgewandt), plus a rule: under 3 kept items or over half dropped → hint. Shown with ⚠ in push and Schrank-Fotos
...
- Hexagon v69 correction (2026-09-21): llama.cpp still has no v69 backend, but ExecuTorch + QNN does. github.com/avisre/snapdragon-npu-llm ships a prebuilt bundle (`K9FxNa/Qwen3-0.6B-SM8450-Hybrid`, 887 MB, ExecuTorch `.pte` + `libQnnHtpV69Skel.so`), SM8475 listed as working via the SM8450 bundle, runs from the adb shell in `/data/local/tmp` with `LD_LIBRARY_PATH`/`ADSP_LIBRARY_PATH`. Claimed on 8 Gen 1: 31 tok/s decode, 103 tok/s prefill
- [x] Verified on the Poco 2026-09-21: Qwen3-0.6B on Hexagon v69 via ExecuTorch+QNN, prefill 242 tok/s, decode 28 tok/s, TTFT 95 ms, RSS 1 GB. Their `install.sh` breaks on the "+" in "8+ Gen 1" and on pip; done by hand: curl the three files on the Poco into `/sdcard/npu`, `adb shell cp` to `/data/local/tmp/executorch_qualcomm_tutorial`, run with `LD_LIBRARY_PATH=$PWD ADSP_LIBRARY_PATH=$PWD`. Output quality of a 0.6B thinking model in German: unusable
- 8 MB VTCM does not cap model size, v73 has the same 8 MB and runs 3B. It is activation scratch. The real v69 difference is no INT4 HMX; w4a16 still compiles for v69 (AI Hub `--precision w4a16 --device "Samsung Galaxy S22 5G"`), dequant on the fly
- Bigger models = compile it yourself. Both paths need a Qualcomm account: AI Hub cloud compile (API token, SaaS) or local ExecuTorch with the QAIRT + Hexagon SDKs (account only for the download, compile offline). Local fits the no-SaaS rule. Listed decoders: llama3.2 1B/3B, qwen2.5 1.5B, qwen3 0.6B, phi-3.5-mini 3.8B, gemma3 1B. No VLM recipe for v69
- Limits that matter for mealprep: v69 has 8 MB VTCM, so sub-1B models only, and no VLM on v69 (that starts at SM8550). Receipts and fridge photos stay on the CPU. A 0.6B text model is too small for the German recipe prompt, so suggestions stay on E4B too. NPU = a fast toy here, not a path for this app
